...
export const AdminDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    setCadetUsers((prev) =>
      prev.map((c) => {
        ...
        return {
          ...
          section: options.section || c.section || 'Section 01',
          rank: options.rank || c.rank || 'Cadet (CDT)',
          status: targetCategory === 'Ex-cadets' ? 'Alumni' : 'Active',
          cadetType: targetCategory === 'Ex-cadets' ? 'Ex-cadet' : 'Current',
          isApproved: true,
        };
      })
    );
  ...
  const cadetLogin = (cadetNo: string, pass: string): { success: boolean; message: string; cadet?: CadetUserAccount } => {
    ...
    if (!found.isApproved || found.status === 'Pending Approval') {
      return {
        success: false,
        message: 'Your registration is currently pending review by Platoon Administration. Once approved, Admin will provide your password.',
      };
    }
  ...
  const cadetRegister = (cadetData: Partial<CadetUserAccount>): { success: boolean; message: string; cadet?: CadetUserAccount } => {
    ...
    const proposedId = cadetData.cadetNo?.trim().toUpperCase() || cadetData.collegeId?.trim().toUpperCase() || `APPL-${Math.floor(1000 + Math.random() * 9000)}`;
    const category: PlatoonCategory = cadetData.category || (cadetData.gender === 'Female' ? 'Female Platoon' : 'Male Platoon');

    const newApplicant: CadetUserAccount = {
      id: `usr-${Date.now()}-${Math.floor(100 + Math.random() * 900)}`,
      cadetNo: proposedId,
      password: '', // Kept empty until Admin reviews and provides password
      name: cadetName,
      category,
      section: cadetData.section || (category === 'Band Platoon' ? 'Band Section 01' : 'Section 01'),
      rank: cadetData.rank || 'Cadet (CDT)',
      gender: cadetData.gender || (category === 'Female Platoon' ? 'Female' : 'Male'),
      appointment: cadetData.appointment || 'Recruit Trainee',
      platoon: category,
      batch: cadetData.batch || 'Batch 24',
      collegeId: cadetData.collegeId || proposedId,
      department: cadetData.department || 'General',
      bloodGroup: cadetData.bloodGroup || 'O+',
      ...
      cadetType: category === 'Ex-cadets' ? 'Ex-cadet' : 'Current',
      isApproved: false,
    };

    setCadetUsers((prev) => [newApplicant, ...prev]);
    return {
      success: true,
      message: 'Registration submitted successfully! Your application has been sent to Platoon Admin for approval. Admin will assign your login password.',
      cadet: newApplicant,
    };
  };
