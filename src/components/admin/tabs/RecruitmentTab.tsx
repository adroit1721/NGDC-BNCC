...
export const RecruitmentTab: React.FC = () => {
  ...
  const handleAddField = (e: React.FormEvent) => {
    ...
    if (setRecruitmentFields) {
      setRecruitmentFields((prev) => [...(prev || []), newField]);
    }
    setNewFieldLabel('');
    setNewFieldOptions('');
  };
  ...
  const handleDeleteField = (id: string) => {
    if (setRecruitmentFields) {
      setRecruitmentFields((prev) => (prev || []).filter((f) => f.id !== id));
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#fcf9f3] dark:bg-[#1e1d19] p-6 rounded-3xl border border-[#cdc6b3]/50 dark:border-[#423e35] shadow-xs">
        <div>
          ...
          <p className="text-xs md:text-sm text-[#695c4e] dark:text-[#aca596] mt-1">
            Publish circulars, design custom applicant forms, export records directly to Excel (.xlsx), and publish selected cadet rolls.
          </p>
        </div>
      ...
      {/* SUBTAB 4: RECRUITMENT FORM BUILDER */}
      {activeSubtab === 'formBuilder' && (
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold text-[#1c1c18] dark:text-[#fcfbf7] flex items-center gap-2">
                  ...
                  <span>Applicant Form Fields Configuration</span>
                </h3>
                <p className="text-xs text-[#7c7767] dark:text-[#aca596]">
                  These custom input fields are dynamically rendered for candidates submitting the public recruitment form.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2.5">
              {recruitmentFields.map((field, idx) => (
                <div
                  key={field.id ? `rf-${field.id}-${idx}` : `rf-${idx}`}
            ...
            <form onSubmit={handleAddField} className="space-y-3 text-xs">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Previous Scouting / Cadet Experience"
                  value={newFieldLabel}
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  ...
                  className="rounded text-[#6b5e10]"
                />
                <label htmlFor="recFldReq" className="font-semibold text-[#1c1c18] dark:text-[#fcfbf7]">
